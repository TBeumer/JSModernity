...
import { getCommitShaByTagName, getReleaseTags } from "./utils/github-api-connector-util.js";
...
const cleanupAndExit = (exitCode) => {
  if (dir) {
    try {
      ...
      console.warn(`Failed to remove temporary files, requires manual deletion... Error: ${e.message}`);
    }
  }
...
const saveResult = (result) => {
  ...
  const fileName = `${dir}/signatures_${owner}_${repo}_${date}`;
  ...
  console.log(`Saving result signature for ${owner}:${repo} to file ${fileName}`);
...
const logTimings = () => {
  ...
  const maxKeyLength = Math.max(...Object.keys(finalTimings).map(key => key.length));
...
if (!args.owner) {
  console.error("Error: Missing owner name. Please provide an owner name using the --owner=<NAME> argument...");
  cleanupAndExit(1);
}
if (typeof args.owner !== "string") {
  console.error("Error: Owner name must be a string. Please provide a valid owner name using the --owner=<NAME> argument...");
  cleanupAndExit(1);
}
...
if (!args.repo) {
  console.error("Error: Missing repository name. Please provide a repository name using the --repo=<NAME> argument...");
  cleanupAndExit(1);
}
if (typeof args.repo !== "string") {
  console.error("Error: Repository name must be a string. Please provide a valid repository name using the --repo=<NAME> argument...");
  cleanupAndExit(1);
}
...
if (typeof args.maxDate !== "string" || isNaN(Date.parse(args.maxDate))) {
  console.error("Error: Maximum date must be a string in ISO 8601 format. Please provide a valid maximum date using the --maxDate=<DATE> argument...");
  cleanupAndExit(1);
}
...
if (!cloneRepo(dir, owner, repo)) {
  console.error("Error: Something went wrong while cloning the repository. There is likely additional logging above. Exiting...");
  cleanupAndExit(1);
}
...
if (!releaseTags) {
  console.error("Error: Something went wrong while fetching release tags. There is likely additional logging above. Exiting...");
  cleanupAndExit(1);
}
...
const result = createEmptyResultObject({ owner, repo, maxDate: maxDate.toISOString() });
for (const release of releaseTags) {

  // Get commit sha for release tag
  console.log("Fetching commit sha for release tag:", release.tag, "|", release.date);
  let commitSha = null;
  for (let i = 0; i < MAX_RETRIES; i++) {
    commitSha = await getCommitShaByTagName(owner, repo, release.tag);
    if (commitSha) break;
    console.warn("Warning: Failed to fetch commit sha for release tag. Retrying... Attempt:", i + 1, "of", MAX_RETRIES);
  }
  if (!commitSha) {
    console.error("Error: Something went wrong while fetching the commit sha. There is likely additional logging above. Skipping...");
    continue;
  }
  ...
  for (let i = 0; i < MAX_RETRIES; i++) {
    ...
    console.warn("Warning: Failed to checkout commit. Retrying... Attempt:", i + 1, "of", MAX_RETRIES);
  }
  if (!checkoutSuccess) {
    console.error("Error: Something went wrong while checking out the commit. There is likely additional logging above. Skipping...");
    continue;
  }
